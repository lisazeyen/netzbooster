...
#     bus_0 = n.lines.loc[x[line_outage_remove][1]].bus0
    
#     bus_1 = n.lines.loc[x[line_outage_remove][1]].bus1
    
#     buses_x= [i for i in buses if bus_0 in i or bus_1 in i]
...
# 
    ...
    c4 = {(k, l, t):
          ...
          for k in outages for l in lines.difference(pd.Index([k, k.split("_outage")[0]])) for t in snapshots}
    # if bus0[l]==bus0[k] and if bus1[l]==bus1[k]

    l_constraint(m, "LineDn", c4, outages, lines, snapshots)
    

    ######## sum(i, PTDF(l,i) * (p_neg(i,k) - p_pos(i,k)) <= F(l)-F(l_out) + f(l) + LODF(l,k) * f(k) ########
    c4_out = {(k, t):
          [[*[(-ptdf.at[k.split("_outage")[0], i], m.p_pos[i, k, t]) for i in buses],
            *[(ptdf.at[k.split("_outage")[0], i], m.p_neg[i, k, t]) for i in buses
              ]], "<=", n.lines_t.p0.at[t, k.split("_outage")[0]] + lodf.at[k.split("_outage")[0], k] * n.lines_t.p0.at[t, k] + n.lines.at[k.split("_outage")[0], "s_nom"] - n.lines.at[k, "s_nom"]]
          for k in outages for t in snapshots}
    
    l_constraint(m, "LineDn_out", c4_out, outages, snapshots)
    ...
    l_constraint(m, "LineUp", c5, outages, lines, snapshots)
    

    # sum(i, PTDF(l,i) * (p_pos(i,k) - p_neg(i,k)) <= F(l)-F(l_out) - f(l) - LODF(l,k) * f(k)
    c5_out = {(k, t): [[
        *[(ptdf.at[k.split("_outage")[0], i], m.p_pos[i, k, t]) for i in buses],
        *[(-ptdf.at[k.split("_outage")[0], i], m.p_neg[i, k, t]) for i in buses
          ]], "<=", -n.lines_t.p0.at[t, k.split("_outage")[0]] - lodf.at[k.split("_outage")[0], k] * n.lines_t.p0.at[t, k] + n.lines.at[k.split("_outage")[0], "s_nom"] - n.lines.at[k, "s_nom"]]
    ...
    l_constraint(m, "LineUp_out", c5_out, outages, snapshots)
    ...
    objective.variables.extend([(coefficient2, m.p_pos[i, k, t])
                                for i in buses for k in outages for t in snapshots])
    objective.variables.extend([(coefficient2, m.p_neg[i, k, t])
                                for i in buses for k in outages for t in snapshots])
...
def pyomo_postprocess(options=None, instance=None, results=None):
    ...
    p_positive = get_values(m.p_pos)
    p_positive.index.set_names(["bus", "outage", "snapshot"], inplace=True)
    p_positive = p_positive.unstack("bus")
    p_positive.to_csv(snakemake.output.pos)
    
    p_negative = get_values(m.p_neg)
    p_negative.index.set_names(["bus", "outage", "snapshot"], inplace=True)
    p_negative = p_negative.unstack("bus")
    p_negative.to_csv(snakemake.output.neg)
#   p.to_csv("new/p.csv")
