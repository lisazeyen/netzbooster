...
__copyright__ = "Copyright 2021 Amin Shokri Gazafroudi (KIT) (KIT), GNU GPL 3"
...
def add_contingency_constraints(network,snapshots,tatl):
    ...
    for branch in branch_outages:
        ...
        flow_upper.update({(branch[0],branch[1],b[0],b[1],sn) : [[(1,network.model.passive_branch_p[b[0],b[1],sn]),(sub.BODF[sub._branches.at[b,"_i"],branch_i],network.model.passive_branch_p[branch[0],branch[1],sn])],"<=",tatl*sub._fixed_branches.at[b,"s_nom"]] for b in sub._fixed_branches.index for sn in snapshots})

        flow_upper.update({(branch[0],branch[1],b[0],b[1],sn) : [[(1,network.model.passive_branch_p[b[0],b[1],sn]),(sub.BODF[sub._branches.at[b,"_i"],branch_i],network.model.passive_branch_p[branch[0],branch[1],sn]),(-tatl,network.model.passive_branch_s_nom[b[0],b[1]])],"<=",0] for b in sub._extendable_branches.index for sn in snapshots})


        flow_lower.update({(branch[0],branch[1],b[0],b[1],sn) : [[(1,network.model.passive_branch_p[b[0],b[1],sn]),(sub.BODF[sub._branches.at[b,"_i"],branch_i],network.model.passive_branch_p[branch[0],branch[1],sn])],">=",-tatl*sub._fixed_branches.at[b,"s_nom"]] for b in sub._fixed_branches.index for sn in snapshots})

        flow_lower.update({(branch[0],branch[1],b[0],b[1],sn) : [[(1,network.model.passive_branch_p[b[0],b[1],sn]),(sub.BODF[sub._branches.at[b,"_i"],branch_i],network.model.passive_branch_p[branch[0],branch[1],sn]),(tatl,network.model.passive_branch_s_nom[b[0],b[1]])],">=",0] for b in sub._extendable_branches.index for sn in snapshots})
...
def add_contingency_constraints_lowmem(network, snapshots,tatl):
    ...
    for sn in n.sub_networks.obj:
        ...
        for outage in outages:
            ...
            if len(ext_i):
                ...
                lhs[key] = lhs_flow_ext + linexpr((-tatl, s_nom_ext))
                rhs[key] = 0
...
def network_sclopf_tatl(network, snapshots=None, branch_outages=None, solver_name="glpk",
                   pyomo=True, skip_pre=False, extra_functionality=None, solver_options={},
                   keep_files=False, formulation="kirchhoff", ptdf_tolerance=0., tatl=None):
    """
    ...
    def _extra_functionality(network, snapshots):
        if pyomo:
            add_contingency_constraints(network, snapshots,tatl)
        else:
            add_contingency_constraints_lowmem(network, snapshots,tatl)
